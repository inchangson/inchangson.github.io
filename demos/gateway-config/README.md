...
[고정 JSON](2026-09-08-results.json)은 원본 커밋 `2afe725`의 `load-test/results/latest.json`과 동일한 2026-09-08 00:02 KST 측정값이다. 원본에서 `git show 2afe725:load-test/results/latest.json`으로도 확인한다. 실제 Spring HTTP 1,976건과 mock API 1,900건으로 총 3,876건이다. `success`는 HTTP 200 기준이다. raw 요청별 latency는 없고 percentile과 합계가 저장되어 있다. 4편에서 결과를 해설한다.

당시 원본 checkout:

```text
/Users/son-inchang/Work/mobility/backup/GatewayPoC/sp-gw-mgmt
```

기준 커밋 `2afe725`. 원본이 있다면 루트에서 `./load-test/scripts/run.sh`로 재실행한다. Java 17, Python 3, curl과 Gradle 의존성이 필요하다. 8888/18080 포트를 사용하고 `load-test/fixtures`를 다시 만든다. 이 스크립트는 원본 프로젝트가 필요하므로 블로그에서 직접 실행하는 명령이 아니다.

| 근거 | 원본 루트 기준 경로 |
|---|---|
| wrapper 등록 | `src/main/java/com/avis/sp_gw_mgmt/StateAwareNativeConfigServerConfiguration.java` |
| mtime·cache·fallback | `src/main/java/com/avis/sp_gw_mgmt/StateAwareNativeEnvironmentRepository.java` |
