...
Gateway는 뒤쪽 서비스와 설정 서버에 연결되어야 요청을 처리한다. 그래서 로컬에서 JAR가 실행되는 것만으로 배포 작업을 설명하기 어렵다. `sp-gw`에서는 Skaffold·Helm을 이용한 로컬 개발 구성과 Jenkins의 개발계 빌드·이미지 배포 경로를 만들었다.

분석 기준은 앞선 글과 같은 `9c7dc07`이다. 2025년 9월에는 로컬 빌드·배포 구성을 추가했고, 11월에는 외부 설정·RabbitMQ 연결을 보강했으며, 12월에는 개발계 Helm과 Jenkins 구성을 추가했다. 현재 소스에서 확인되는 작업 기간이 12월까지 이어지는 이유다.
...
원본 루트는 `/Users/son-inchang/Work/mobility/backup/GatewayPoC/sp-gw`다. 아래 파일을 순서대로 읽으면 환경 설정이 실행 프로세스로 전달되는 경로를 따라갈 수 있다.

1. `skaffold.yaml`: 로컬 build·deploy·portForward
2. `charts/local-values.yaml`, `charts/ipcdev-values.yaml`: 환경별 입력
3. `charts/sp-gw/templates/deployment.yaml`: 입력이 실제로 사용되는 위치
4. `src/main/resources/application.yaml`: 프로세스에서 읽는 설정
5. `cicd/Jenkinsfile_ipcdev`, `Dockerfile.ipcdev`: JAR에서 이미지, CD 호출까지

근거 커밋은 `8011bf2`(Skaffold·Helm), `dbffd20`(Config 주소 분리), `5df3484`(개발계 Helm), `00aa077`(개발계 CI), `6f841f0`(profile 관리)이다. 면접 준비용 소스 맵(`docs/sp-gw-interview/source-map.md`)에 전체 경로와 읽기 전용 확인 명령을 정리했다.
