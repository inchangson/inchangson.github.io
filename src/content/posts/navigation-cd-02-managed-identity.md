...
3개 OEM 사이트의 초기 CD는 Jenkins가 스토리지 계정 키로 배포 스크립트를 받고 DB 비밀번호와 토큰 서명용 비밀값을 조회했다. 이를 SSH 명령문에 넣어 대상 VM의 환경 파일을 만들었다. 나는 이 조회를 대상 WAS VM의 Azure 관리 ID로 옮기고, 전환 중 발생한 CLI 옵션과 인증서 오류를 해결했다.
