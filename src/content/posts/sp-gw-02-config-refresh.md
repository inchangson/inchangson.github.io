...
[1편](/blog/sp-gw-01-request-contract)의 필터는 Java 코드로 구현하지만, 어떤 경로에 붙일지는 라우팅 설정의 책임이다. `gateway-example` 작업에서는 이 설정을 Config Server로 모으고 Gateway가 설정을 다시 읽을 경로를 구성했다. 이렇게 하면 이미 구현된 필터의 인자나 서비스 주소를 바꾸기 위해 애플리케이션 JAR를 다시 만들 필요가 줄어든다.

이 글은 `gateway-example`의 클라이언트 코드와 변경 이력을 분석한다. 별도 Config Server의 저장소 구현, 파일 캐시, 파일 변경 감지는 이 저장소만으로 확인할 수 없다. 따라서 이 글에서 설명하는 범위는 Gateway 쪽 설정 수신과 갱신이다.

## 설정의 위치를 한 군데로 모으다

애플리케이션 YAML에 있던 route 정의를 외부 설정으로 옮겼다. 이전 파일에는 서비스별 URI, Path predicate, 인증 필터와 Rewrite 규칙이 함께 있었다. 현재 파일에는 아래 import와 애플리케이션 이름이 남아 있다.
...
this.refreshUri = "http://" + host + ":" + port
    + "/gateway-example/actuator/refresh";
...
host와 port는 Environment에서 읽으며 기본값은 localhost와 8080이다. 기준 코드의 주기는 1시간이다. 주기와 base path 변경에 맞춰 자기 호출 설정도 조정했다. base path를 바꾸면 내부 self-call URL도 같이 점검해야 한다는 사례다.
...
비공개 이력에서 `spring-cloud-starter-bus-amqp` 의존성과 RabbitMQ 접속 설정을 추가했다. Deployment는 RabbitMQ의 host·port·사용자·비밀번호를 Secret에서 환경 변수로 주입한다.
...
| `src/main/java/com/example/apigateway/route/SelfRefreshScheduler.java` | 자기 호출과 주기 |
| `src/main/java/com/example/apigateway/support/WebClientConfig.java` | WebClient 생성 |
| `build.gradle` | Config·Bus 의존성 |
| `charts/gateway-example/templates/deployment.yaml` | RabbitMQ와 Config 주소 주입 |
...
근거 목록(`docs/sp-gw-interview/source-map.md`)에는 공개 검증 범위를, [3편](/blog/sp-gw-03-delivery-boundary)에는 이러한 설정을 배포 환경에 연결한 과정을 정리했다.
