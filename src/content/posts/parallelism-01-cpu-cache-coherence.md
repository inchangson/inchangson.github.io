...
싱글 코어에서도 여러 작업이 함께 진행되는 것처럼 보일 수 있다. 운영체제가 실행할 스레드를 바꾸고, 한 작업이 I/O를 기다리는 동안 다른 작업을 실행하기 때문이다. 같은 시간 구간에 여러 작업의 진행이 겹치는 **동시성(concurrency)** 이지만, 한 시점에 한 코어에서 실행되는 명령 흐름은 하나다.
...
|---|---:|---:|---|
| 싱글 코어에서 여러 runnable 스레드 | 가능 | 불가능 | 스케줄링, 전환, 캐시 교란 |
