...
재발송 대기, OTP 만료, 발송·검증 상한은 서버 정책으로 판정한다. 실제 정책 수치는 공개하지 않으며 브라우저 countdown이 보안 판정을 대신하지 않는다.
...
비밀번호 저장은 기존 로그인과의 호환 계약을 유지했다. 구체적인 레거시 알고리즘과 구성은 공개하지 않는다. 브라우저의 추가 암호화는 TLS나 안전한 password KDF를 대체하지 않으며, 저장 계약은 버전을 구분해 단계적으로 이행해야 한다.
...
결과적으로 가입 때 검증한 번호를 이후 복구 흐름의 신뢰값으로 사용할 기반을 개발계에서 확인했다. 남은 과제는 레거시 password hash를 modern KDF로 이행하고, OTP 정책과 rollout 대상을 중앙에서 관리하는 것이다.
