...
비밀번호 저장은 기존 로그인과 호환해야 해서 legacy password hash 기반 계약을 그대로 사용했다. 이는 현대적인 password KDF 강도로 포장할 수 없다. 브라우저에서 서버 세션 공개키로 RSA-OAEP 암호화하는 것은 전송 중 평문 노출을 줄이는 별도 층이며 TLS를 대체하지도, 저장 해시의 약점을 보완하지도 않는다.
