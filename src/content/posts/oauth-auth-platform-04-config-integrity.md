...
- redirect와 PKCE에는 이행을 위한 완화 설정이 남아 있다. 스캔은 지원되는 값인지를 볼 뿐, 모든 client가 가장 엄격한 정책인지 강제하지 않는다. 허용 예외의 만료와 승인 근거를 별도 정책으로 관리해야 한다.
- password는 기존 시스템과 맞춘 legacy SHA-256 호환 계약이다. 무결성 스캔은 credential 관계를 찾지만 해시 강도를 높이지 않는다. 로그인 성공 시 rehash 같은 단계적 modern KDF 이행이 필요하다.
