...
구현 근거는 `mob_agent/bo-auth`의 `e586273f796242f94208a661aed9efe56f0f07a4` 기준이다. 아래 Java 경로는 `src/main/java/com/kt/ivi/avis/bo/auth/` 아래다. 비공개 업무 코드는 경로와 메서드로 표시하고, 일반 원리는 공개 문서로 연결했다.
...
“정책 선택 구조에 plain과 비활성, redirect 완화 설정도 남아 있습니다. 따라서 S256 경로를 구현했다는 사실과 모든 설정 조합이 안전하다는 주장을 구분합니다. 운영 전에는 허용할 정책을 좁히고 잘못된 조합을 시작 시점에 거부하는 조건이 필요합니다.”
...
“새 세션을 만들며 우회할 수 있습니다. 복구에서는 로그인 세션, 채널, 연락처의 해시 키를 기준으로 DB upsert 카운터를 추가했습니다. 다만 로그인 세션까지 교체하는 공격에 대한 전역 제한이라고 설명하면 안 됩니다. 계정, IP, 기기별 추가 제한은 별도 설계 대상입니다.”
...
“현재 `PasswordResetService.hashPassword`는 기존 `KeyUtils.hashPassword` 계약을 호출합니다. 호환성을 유지한 것이고 SHA-256 기반 저장을 개선한 경험은 아닙니다. 새 버전의 해시 형식을 구분해 저장하고, 로그인 성공 시 Argon2id 같은 password KDF로 재해시하는 이행안을 검토하겠습니다. 로그인과 가입, 복구가 같은 형식을 처리해야 합니다.” 일반 SHA-256처럼 빠른 해시는 비밀번호 대입 공격 비용을 충분히 높이지 못하므로 password KDF를 써야 한다. [OWASP Password Storage](https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html)

## 면접 직전 확인할 세 가지

- 수치 질문에는 개발계 검증 범위를 답한다. 운영 TPS와 사고 감소율은 측정하지 않았다.
- 이메일 복구의 계정 정본과 credential 변경은 SMS 경로와 같다고 설명하지 않는다.
- 기존 SHA-256 저장 계약의 호환성을 유지한 것이며, 현대적인 password KDF로 개선한 경험은 아니다.
