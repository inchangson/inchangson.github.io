...
이 글에서는 이를 **OAuth 2.1 지향** 경로라고 부른다. 정확히는 OAuth 2.0 Authorization Code 흐름에 PKCE와 [RFC 9700의 OAuth 2.0 보안 권고](https://www.rfc-editor.org/info/rfc9700/)를 적용한 구현이다. OAuth 2.1은 현재도 [IETF Internet-Draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/)이며, 구현에는 redirect와 PKCE를 완화할 수 있는 설정도 남아 있다. 따라서 완전 준수를 주장하지 않는다.
...
| 관심사 | 대표 선택 | 실패 시 의미 |
|---|---|---|
| redirect 검증 | 정확 일치, 등록 scheme 허용, 완화 모드 | 잘못된 목적지로 code 유출 가능 |
| PKCE | S256 필수, plain, 비활성 | 탈취된 code의 재사용 위험 |
| scope | 정확 일치, 허용 범위 부분집합, 완화 | 과도한 권한 발급 가능 |
...
자체 구현은 도메인 흐름을 코드에서 직접 통제하게 해 주었지만, 표준 프레임워크가 이미 해결한 프로토콜 세부와 보안 업데이트를 스스로 추적해야 한다. 완화 가능한 redirect/PKCE 기본값도 강제 정책으로 전환해야 한다. 이번 결과는 “OAuth 2.1 완전 구현”이 아니라, **인증원 변경을 견디는 인가 경계를 만들고 개발계에서 끝까지 검증한 것**이다.
