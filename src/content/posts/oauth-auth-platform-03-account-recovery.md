...
재발송 cooldown과 횟수는 하나의 복구 세션 안에서 잘 작동한다. 그러나 공격자가 매번 새 복구 세션을 만들면 모두 처음 발송이다. 그래서 복구 세션을 만들기 전에 “로그인 세션 + 채널 + 연락처”의 해시 키로 별도 발송 창을 기록했다.
...
비밀번호 계산은 가입·로그인이 사용하는 기존 함수를 그대로 호출했다. 이 시스템의 저장 계약은 legacy SHA-256 조합이다. 호환성은 확보하지만 modern password KDF와 같은 강도는 아니다. 별도 마이그레이션 전략 없이 복구 경로만 알고리즘을 바꾸면 로그인과 불일치하므로, 이번에는 계약 유지가 우선이었다.
