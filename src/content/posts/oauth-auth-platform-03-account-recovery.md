...
비밀번호 계산은 가입·로그인이 사용하는 기존 호환 계약을 유지했다. 구체적인 레거시 알고리즘과 구성은 공개하지 않는다. 복구 경로만 단독으로 바꾸면 로그인과 불일치하므로, 버전 구분과 로그인 성공 시 rehash를 포함한 modern password KDF 이행이 필요하다.
...
외부 메시지 제공자와 포털/BFF까지 연결한 개발계 E2E도 수행했다. 운영 전 검토 항목은 계정 정본 정의, modern password KDF 이행, 알림 실패의 재처리·관측 체계다. 환경별 완료 여부와 외부 연동 세부는 공개하지 않는다.
