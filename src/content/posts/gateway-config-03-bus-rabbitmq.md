...
부하 실험의 client 수는 합성 시나리오의 조건이다. 실제 플랫폼 구성과 Bus 구독자 수는 공개하지 않는다.
...
| 배포 시 환경변수 주입 | `charts/config-server-example/templates/deployment.yaml` |
| Secret 템플릿 | `charts/config-server-example/templates/rabbitmq-secret.yaml` |
| Kafka → RabbitMQ 이력 | 비공개 변경 이력 |
