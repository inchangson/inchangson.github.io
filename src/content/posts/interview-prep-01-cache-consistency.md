...
코드 근거는 `sp-gw-mgmt@2afe725`의 `src/main/java/com/avis/sp_gw_mgmt/StateAwareNativeEnvironmentRepository.java`다. `cache`, `findOne`, `getFileInfo`, `buildCacheKey`를 함께 읽으면 조회 순서가 보인다. 캐시 도입은 `cf26314`, 키 구성 변경은 `7d072d0`이며 `2afe725`는 분석과 실험 기준이다. 실험 데이터는 블로그 저장소의 `demos/gateway-config/2026-09-08-results.json`에 있다.
...
클라이언트 근거는 `sp-gw@9c7dc07`의 `src/main/java/com/avis/apigateway/route/SelfRefreshScheduler.java`와 `src/main/resources/application.yaml`이다. Bus 의존성 추가는 `26d6627`, 주기 변경은 `8c5e611`이다. 서버 파일 변경을 자동으로 감시해 Bus를 호출하는 기능은 확인되지 않았다.

## 직접 확인할 자료

원본 저장소는 로컬의 `Work/mobility/backup/GatewayPoC/sp-gw-mgmt`와 `sp-gw`다. 비공개 코드는 위 파일 경로와 커밋으로 추적하며, 공개 저장소 링크를 추정해 만들지 않았다.
