...
> 이 디렉터리는 완성된 성공담이 아니라, SA01의 ATAM·Telecop 변경을 실제 커밋과 Java 8 재현 실험으로 다시 이해하는 작업 기록이다.

## 먼저 바로잡은 한 문장

현재 이력서에는 다음 문장이 있다.

> ATAM·Telecop 호출을 공통 Sender와 RestTemplate으로 통합하고 연동 로그·예외 규격을 표준화

코드에서 바로 확인되는 표현은 조금 다르다.

- ATAM과 Telecop은 하나의 Sender가 아니라 연동처별 Sender로 분리되었다.
...
> ATAM·Telecop 발신 로직을 연동처별 Sender로 분리하고, 공통 RestTemplate 기반 전송과 요청·응답 로그 경로를 정리했다.

## 조사 범위

| 구분 | 기준 | 사용 목적 |
|---|---|---|
| 주 변경 구간 | SA01 `092eea1..9a55c9c` | Sender 분리와 HTTP·로그 공통화 과정 |
| 종료 상태 | `9a55c9c`의 커밋된 blob | 당시 구조 설명 |
| 직후 보완 | `05bfd63`, `b51e8d64`, `e75c23f4` | 정리 직후 발견된 파라미터·호출 시점 보완 |
...
```mermaid
flowchart LR
    A["기존 Service와 PushSender에<br/>연동 코드 분산"] --> B["Telecop Sender 생성"]
    B --> C["ATAM Sender 생성"]
