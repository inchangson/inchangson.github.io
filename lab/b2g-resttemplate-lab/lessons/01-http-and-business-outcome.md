...
- PartnerA 종료점 코드는 주로 HTTP `200`을 성공으로 판정한다.
- PARTNER_B 종료점 코드는 HTTP 상태와 body의 `resltCd`를 나눠 읽는다.
- `HttpConnectionUtils`는 HTTP 오류 응답은 Map으로 바꾸지만 read timeout처럼 HTTP 상태가 없는 실패는 그대로 전파한다.

## 재현 조건과 관찰 결과

아래 표는 축소 데모의 결과다. 데모 PartnerA Sender는 **공통 전송·로그 함수 경계**를 드러내기 위해 실제 public Sender의 바깥 catch를 생략했다. 실제 B2G public Sender는 전송 예외를 서버 로그에 기록하고 false로 반환하는 경로가 있다. 데모에서 예외가 전파됐다고 운영 API까지 같은 예외가 전파됐다고 해석하면 안 된다. PARTNER_B 데모의 boolean 반환도 비교용이다. 실제 PARTNER_B의 일부 public 발신 메서드는 void이며 내부 오류 코드·로그 경로를 사용한다.

| 입력 | PartnerA | PARTNER_B | 로그 |
|---|---|---|---|
